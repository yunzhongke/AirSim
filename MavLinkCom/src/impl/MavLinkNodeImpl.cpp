...
void MavLinkNodeImpl::sendCommand(MavLinkCommand& command)
{
    ...
    try {
        ...
        throw std::runtime_error("MavLinkNodeImpl.cpp : sendCommand -> sendMessage fail..");
    }
}
        ...
        if (message.msgid == MavLinkCommandAck::kMessageId)
        {
            ...
            if (ack.command == cmd)
            {
                ...
                if (ackResult == MAV_RESULT::MAV_RESULT_TEMPORARILY_REJECTED) {
                    Utils::log(Utils::stringf("\n### command %d result: MAV_RESULT_TEMPORARILY_REJECTED", cmd));
                }
                else if (ackResult == MAV_RESULT::MAV_RESULT_UNSUPPORTED) {
                    Utils::log(Utils::stringf("\n### command %d result: MAV_RESULT_UNSUPPORTED", cmd));
                }
                else if (ackResult == MAV_RESULT::MAV_RESULT_FAILED) {
                    Utils::log(Utils::stringf("\n### command %d result: MAV_RESULT_FAILED", cmd));
                }
                else if (ackResult == MAV_RESULT::MAV_RESULT_ACCEPTED) {
                    Utils::log(Utils::stringf("\n### command %d result: MAV_RESULT_ACCEPTED", cmd));
                }
                else {
                    Utils::log(Utils::stringf("\n### command %d unexpected result: %d", cmd, ackResult));
                }
